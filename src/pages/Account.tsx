  ...
  const handleExportData = async (format: 'json' | 'csv' = 'json') => {
    ...
    try {
      ...
      const [logsRes, protosRes, reviewsRes, communityRes, subsRes, wellnessRes, labRes, sideEffectRes, profileRes] = await Promise.all([
        trackQuery(supabase.from('injection_logs').select('id, peptide_name, dose, dose_unit, injection_site, notes, logged_at, protocol_id, created_at').eq('user_id', user.id).limit(EXPORT_LIMIT)),
