...
/** Cache Google provider availability (checked once per page load) */
let _googleEnabled: boolean | null = null;
async function isGoogleEnabled(): Promise<boolean> {
  if (_googleEnabled !== null) return _googleEnabled;
  try {
    const res = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/auth/v1/settings`,
      { headers: { apikey: import.meta.env.VITE_SUPABASE_ANON_KEY } },
    );
    if (!res.ok) return (_googleEnabled = false);
    const data = await res.json();
    return (_googleEnabled = data?.external?.google === true);
  } catch {
    return (_googleEnabled = false);
  }
}
...
export default function Login() {
  ...
  const [googleAvailable, setGoogleAvailable] = useState(true); // optimistic
  const { login, signup, user } = useAuth();

  useEffect(() => { isGoogleEnabled().then(setGoogleAvailable); }, []);
    ...
    <div className="flex min-h-screen items-center justify-center bg-white px-4">
      ...
      <div className="w-full max-w-md">
        <div className="overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-lg">
          ...
          <div className="px-6 pb-8 pt-6">
            {/* Google Sign In — hidden when provider is disabled */}
            {googleAvailable && (
              <>
                <button
