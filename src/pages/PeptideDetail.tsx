      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {posts.map((post) => (
          ...
          >
            ...
            <p className="mt-3 text-xs text-stone-500 dark:text-stone-300">{new Date(post.published_at).toLocaleDateString('ar-SA', { year: 'numeric', month: 'short', day: 'numeric' })}</p>
          </Link>
        ))}
      </div>
    </div>
